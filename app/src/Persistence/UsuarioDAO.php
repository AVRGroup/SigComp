...
class UsuarioDAO extends BaseDAO
{
    ...
    public function getEmailByMatricula($matricula)
    {
        try{
            $sql = "SELECT email FROM usuario WHERE usuario.matricula = $matricula";
            $stmt = $this->em->getConnection()->prepare($sql);
            $stmt->execute();
            $results =  $stmt->fetchAll();
        } catch(\Exception $e){
    ...
    {
        $sql = "SELECT * FROM periodo_corrente ORDER BY periodo_atual DESC LIMIT 1";
        $stmt = $this->em->getConnection()->prepare($sql);
        $stmt->execute();
        $periodoCorrente = $stmt->fetchAll();

        return $periodoCorrente[0]['periodo_atual'];
    }
